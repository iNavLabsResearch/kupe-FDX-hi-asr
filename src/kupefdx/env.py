"""Environment, HF auth, repo create/push, wandb init, logging.
...
So the smoke test runs fully offline with no tokens.
"""
...
log = logging.getLogger("kupe-fdx")
...
def require_token() -> str:
    tok = hf_token()
    if not tok:
        raise RuntimeError("HF_TOKEN not set. Put it in .env or: export HF_TOKEN=hf_xxx")
    return tok
...
def ensure_repo(repo_id: str, repo_type: str, private: bool = True) -> str:
...
def upload_folder(folder: str, repo_id: str, repo_type: str, path_in_repo: str = "",
                  commit_message: str = "upload", ignore_patterns=None) -> None:
